...
const float GRID_RESOLUTION = 0.1f; // 网格分辨率 0.5 米 0.1
const int X_MIN = -10, X_MAX = 10, Y_MIN = -10, Y_MAX = 10, Z_MIN = -5, Z_MAX = 5;
const int ScanNumThres = 7; // #big——100 small——30
...
struct Grid {
    ...
    std::vector<std::tuple<int, int, int, std::vector<int>>> getGridsWithMultipleValues() {
        ...
        for (int ix = 0; ix < data.size(); ++ix) {
            for (int iy = 0; iy < data[ix].size(); ++iy) {
                for (int iz = 0; iz < data[ix][iy].size(); ++iz) {
                    // std::cout << "data_size = " << data[ix][iy][iz].size() << std::endl;
        ...
        //     for (auto &y : x)
        //         for (auto &z : y)
        //             maxSize = std::max(maxSize, z.size());
...
int main(){
    ...
    // 测试网格中的帧数
    if(1){
        std::cout << "VoxValueNum=" << grid.data[117][111][55].size() << std::endl; //墙面
        std::cout << "VoxValueNum=" << grid.data[109][129][49].size() << std::endl;  // 罐体
        std::cout << "VoxValueNum=" << grid.data[118][67][46].size() << std::endl;


    }

    // for (const auto& value : grid.data[83][82][54]) {
    for (const auto& value : grid.data[123][118][47]) {
        std::cout << value << " ";
    }
    std::cout << std::endl;
